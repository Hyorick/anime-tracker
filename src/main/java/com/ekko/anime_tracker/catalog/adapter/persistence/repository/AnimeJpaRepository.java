...
class AnimeJpaRepository implements AnimeRepository {
  ...
  public AnimeJpaRepository(
      SpringDataAnimeRepository animeRepository,
      SpringDataGenreRepository genreRepository,
      SpringDataStudioRepository studioRepository,
      AnimeEntityMapper animeEntityMapper) {
  ...
  public List<Anime> findByGenreName(String genreName) {
    return animeRepository.findByGenres_NameIgnoreCase(genreName).stream()
  ...
  public List<Anime> findByStudioName(String studioName) {
    return animeRepository.findByStudio_NameIgnoreCase(studioName).stream()
        .map(animeEntityMapper::toDomain)
        .toList();
  }
  ...
  public List<Anime> findAll() {
    return animeRepository.findAll().stream().map(animeEntityMapper::toDomain).toList();
  }

  @Override
  public Page<Anime> findAll(Pageable pageable) {
    return animeRepository.findAll(pageable).map(animeEntityMapper::toDomain);
  }

  @Override
  public Page<Anime> findAll(Specification<AnimeEntity> specification, Pageable pageable) {
    return animeRepository.findAll(specification, pageable).map(animeEntityMapper::toDomain);
  }
