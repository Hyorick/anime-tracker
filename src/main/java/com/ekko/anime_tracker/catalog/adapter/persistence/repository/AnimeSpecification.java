...
public class AnimeSpecification {
  ...
  public static Specification<AnimeEntity> filter(String title, String genre, String studio) {
    ...
    return (root, query, cb) -> {
      ...
      if (studio != null && !studio.isBlank()) {
        ...
        predicates.add(cb.equal(cb.lower(studioJoin.get("name")), studio.toLowerCase()));
      }

      if (genre != null && !genre.isBlank()) {
        Join<AnimeEntity, GenreEntity> genres = root.join("genres");
        predicates.add(cb.equal(cb.lower(genres.get("name")), genre.toLowerCase()));
      }
